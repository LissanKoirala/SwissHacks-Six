...
output for an actual case-study client**, generated by the backend over the
SwissHacks case-study data (the provided CRM + Portfolio workbooks) — see
`src/content.ts`. To refresh it, re-run the extraction against `get_insights` /
...
else a sliver of an RM) → Solution (dual output) → **the product, on a real
case-study client**: morning desk → the client's trigger → strategy proposal
